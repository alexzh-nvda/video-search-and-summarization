...
# =============================================================================
# Alert Bridge P1 Sanity Check - Master Orchestrator
# =============================================================================
#
# Usage:
#   ES_HOST=localhost ./run_p1.sh              # HTTP-only mode
#   ES_HOST=localhost SSH_HOST=localhost ./run_p1.sh  # Full mode
#   ES_HOST=localhost ./run_p1.sh --json       # JSON output
#   ES_HOST=localhost ./run_p1.sh --verbose    # Verbose mode
#
# Exit Codes:
...
# Initialize results file BEFORE sourcing common.sh (common.sh reads SANITY_RESULTS_FILE)
export SANITY_RESULTS_FILE="/tmp/sanity_p1_results_$$"
...
[[ -f "$SCRIPT_DIR/checks_p1/config_p1.env" ]] && source "$SCRIPT_DIR/checks_p1/config_p1.env"
...
if [[ "$JSON_OUTPUT" != "true" ]]; then
    echo ""
    echo "╔════════════════════════════════════════════════════════════════════╗"
    echo "║                 ALERT BRIDGE P1 SANITY CHECK                       ║"
    echo "╠════════════════════════════════════════════════════════════════════╣"
...
for check in "$SCRIPT_DIR/checks_p1"/*.sh; do
    [[ -f "$check" ]] || continue
