...
# Test: VST Segment Window — End Anchor
# Description: Verify segment_anchor=end computes correct VST time window (startTime=T+20s, endTime=T+30s)
set -euo pipefail
...
TEST_NAME="vst_segment_end"
ID_SUFFIX="p1_${TEST_NAME}_$(date +%H%M%S)"

echo "=== P1: VST Segment Window (end anchor) ==="

mkdir -p "$PID_DIR"

# 1. Create payload with known timestamps (30s apart, both in the past to avoid future-clamping)
# Use unauthorized_entry category which has segment_anchor=end in alert_type_config.json
...
# Use unauthorized_entry which has per-alert-type segment_anchor=end
data['category'] = 'unauthorized_entry'
...
# Expected for anchor=end, duration=10s
expected_start = incident_end - timedelta(seconds=10)
expected_end   = incident_end
