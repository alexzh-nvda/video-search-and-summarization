...
# Test: VST Segment Window — Start Anchor
# Description: Verify segment_anchor=start computes correct VST time window (startTime=T, endTime=T+10s)
set -euo pipefail
...
TEST_NAME="vst_segment_start"
ID_SUFFIX="p1_${TEST_NAME}_$(date +%H%M%S)"

echo "=== P1: VST Segment Window (start anchor) ==="

mkdir -p "$PID_DIR"

# 1. Create payload with known timestamps (30s apart, both in the past to avoid future-clamping)
# Use collision category which has segment_anchor=start in alert_type_config.json
...
# Use collision which has per-alert-type segment_anchor=start
data['category'] = 'collision'
...
# 3. Produce to Kafka (--no-patch to preserve custom timestamps for segment window test)
...
# Expected for anchor=start, duration=10s
expected_start = incident_start
expected_end   = incident_start + timedelta(seconds=10)

log_line = '''$VST_LOG_LINE'''

# Match 'start=<ISO>' and 'end=<ISO>' from the Requesting log,
# or 'effective_start=<ISO>' and 'effective_end=<ISO>' from the window log
