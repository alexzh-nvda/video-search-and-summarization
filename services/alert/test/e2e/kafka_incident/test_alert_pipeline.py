...
"""End-to-end validation for alert (Behavior) processing via Kafka."""
...
def test_kafka_alert_flow(
    ...
    This test validates the complete alert processing pipeline:
    1. Sends an Alert (Behavior) protobuf message to Kafka
    2. Alert Bridge consumes and processes the message
    3. VLM enrichment is applied via NIM simulator
    4. Result is written to Elasticsearch
    5. Test validates the output document fields
    """
    repo_root = Path(__file__).resolve().parents[3]
    payload_path = repo_root / "test/protobuf/test_data/sample_alert.json"
    ...
    try:
        # Use produce_alert.py to send Behavior protobuf to mdx-alerts topic
        producer_cmd = [
            _python_executable(),
            str(repo_root / "test/protobuf/produce_alert.py"),
            "--bootstrap",
            kafka_config["bootstrap"],
            "--topic",
            kafka_config.get("alert_topic", "mdx-alerts"),
            "--payload",
            str(payload_path),
            "--id-suffix",
            "pytest",
        ...
        try:
            ...
            pytest.fail(f"Alert producer failed: {exc.stderr}\nService logs:\n{logs}")
        ...
        # Check multiple possible index dates (today and alert timestamp date)
        # sample_alert.json has timestamp: "2025-10-30T15:30:00Z"
        possible_indices = [
            f"mdx-vlm-alerts-{time.strftime('%Y-%m-%d')}",
            "mdx-vlm-alerts-2025-10-30",  # Sample alert timestamp
        ]
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if documents:
                            ...
                            # Validate sensor.id (nested field in Behavior messages)
                            sensor = first.get("sensor", {})
                            assert sensor.get("id"), "sensor.id should be present"
                            
                            # Validate event.type (nested field in Behavior messages)
                            event = first.get("event", {})
                            assert event.get("type"), "event.type should be present"
                            ...
                            print(f"[test] SUCCESS: Alert document verified with verdict={info.get('verdict')}")
                            return
                except requests.RequestException:
                    pass
            time.sleep(2)
        ...
        logs = _service_logs(service)
        pytest.fail(
            "Timed out waiting for alert document in Elastic simulator\n"
